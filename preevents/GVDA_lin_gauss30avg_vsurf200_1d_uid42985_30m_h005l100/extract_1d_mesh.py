...
qs = np.zeros((nz, 1))
for k in range(nz):
    vp[k] = mesh[k, site_y, site_x, 0] * np.mean(mesh[k, :, :, 0]) / np.tensordot(mesh[k, :, :, 0], gauss) 
    vs[k] = mesh[k, site_y, site_x, 1] * np.mean(mesh[k, :, :, 1]) / np.tensordot(mesh[k, :, :, 1], gauss) 
#vs[k] = np.max((200, mesh[k, site_y, site_x, 1] * np.mean(mesh[k, :, :, 1]) / np.tensordot(mesh[k, :, :, 1], gauss)))
    ro[k] = mesh[k, site_y, site_x, 2] * np.mean(mesh[k, :, :, 2]) / np.tensordot(mesh[k, :, :, 2], gauss) 
    qp[k] = mesh[k, site_y, site_x, 3] * np.mean(mesh[k, :, :, 3]) / np.tensordot(mesh[k, :, :, 3], gauss) 
    qs[k] = mesh[k, site_y, site_x, 4] * np.mean(mesh[k, :, :, 4]) / np.tensordot(mesh[k, :, :, 4], gauss) 

np.savetxt('mesh_gvda_4m_gauss30avg_vsurf200_1d.txt', np.hstack([vp, vs, ro, qp, qs]), fmt='%f')
nxx = 80
nyy = 80
site_xx = 40
site_yy = 41
...
dat.tofile('mesh_gvda_4m_gauss30avg_vsurf200_1d.bin')
...
ax.set_aspect(2)
fig.savefig('vs_perpend_GVDA_gauss30avg_vsurf200_1d.svg', dpi=600, bbox_inches='tight',
                        pad_inches=0.1)
...
ax.set_ylabel('Depth (km)')
ax.set_aspect(2)
fig.savefig('vs_horizon_GVDA_gauss30avg_vsurf200_1d.svg', dpi=600, bbox_inches='tight', pad_inches=0.1)
