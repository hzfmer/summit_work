   ...
   def readsurf(self, ti):
      ...
      for m in range(self.nz):
        for n in range(self.ny):
          ...
          self.V[m,n,:]=np.frombuffer(sr, dtype='f')
...
srf=srfvelclass(f'{dirname}/SX_0_{nt:07d}', nx, ny, nz, dh, tskp, wstep, verbose=False)
...
slider=Tk.Scale(root, from_=tskp,to=(nt-tskp), resolution=50 * tskp,
        orient='horizontal', variable=ts)
slider.pack(side="left", expand=1, fill='x')
