...
#        psr[i,j] = sr[j,i,:].max()
#
slip.tofile(f"slip_lin_{module}.bin")

fig, ax = plt.subplots(figsize=(9,3))
ax.plot(np.arange(nx) * dx, psr[0,:])
ax.set_xlabel('Strike (km)')
ax.set_ylabel('PSR (m/s)')
plt.savefig(f"surf_psr_{module}.svg", dpi=600, bbox_inches='tight', pad_inches=0.05)
...
fig = plt.figure(figsize=(9, 5))
ax0 = fig.add_axes([0.1, 0.1, 0.7, 0.8])
cax = ax0.imshow(slip, extent=[0, nx * dx, nz * dx, 0], aspect = 5,
                cmap=cm.hot_r, vmax=10)
#cnt = np.arange(0., 100., 1.)
#ct = ax0.contour(grid[1,:,:]*dx, grid[0,:,:]*dx, slip)#, cnt)
#cb = plt.colorbar(ct, shrink=0.6, extend='both', orientation='horizontal')
#cb.set_label("Rupture start time (s)")
...
plt.savefig(f"plot_slip_{module}.svg", dpi=600, bbox_inches='tight', pad_inches=0.05)
