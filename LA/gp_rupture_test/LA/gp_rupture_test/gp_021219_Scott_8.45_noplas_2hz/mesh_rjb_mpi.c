...
float mindist(int np, float *lon, float *lat, float tlon, float tlat, int * mdi){
    int k;
    float dx, dy, adist;
    float md=1.e13;
...
int main (int argc, char *argv[]){
    int npt, m, k, n, idx=-1;
    int np = 5116, idx=-1;  /* Twice the length of fault */
    int rank, nprocs;
    float *buff;
    float *buff2;
    ...
    fid = fopen("fault_surf_loc.bin", "r");
    ...
    */
    ...
    for (m=rank; m<npt; m+=nprocs){
        md[m] = mindist(np, lon, lat, tlon[m], tlat[m], &idx);
        if (rank == 0) {
            for (k=1; k<nprocs; k++){
