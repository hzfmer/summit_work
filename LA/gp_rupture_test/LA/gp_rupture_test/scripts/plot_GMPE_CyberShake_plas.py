...
def plot_GMPE_CB(M, T, freq):
    ...
    if M > 6:
        skip = 15
    try:
        data_gmpe = np.fromfile(f'sa_GMPE_M{M}_T{T}.bin', dtype='float32').reshape((-1, 3))
    except FileNotFoundError:
        return 0
    dist, sa_median_gmpe, sa_sigma_gmpe = (data_gmpe[:, i] for i in range(3))
    print(f"GPME_M{M}_T{T} loaded!\n") 
    sa_cb = np.fromfile(f'{home_dir}gp_021219_Scott_{M}_plas_' + freq + 
    ...
    plt.tight_layout()
    fig.savefig(f"gp_021219_Scott_M{M}_T{T}_plas_" + freq + "_imshow.png", dpi=400, bbox_inches='tight', pad_inches=0.05)
    ...
    # x_bins = np.logspace(np.log10(dist.min() + eps), np.log10(dist.max()), np.sqrt(n))
    # y_bins = np.logspace(np.log10(sa_cb.min() + eps), np.log10(sa_cb.max()), np.sqrt(n))
    ...
    fig.savefig(f"gp_021219_Scott_M{M}_T{T}_plas_" + freq + "_vs30.png", dpi=400, bbox_inches='tight', pad_inches=0.05)
