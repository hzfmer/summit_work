...
#para = np.fromfile('../../data_for_Zhifeng/mesh/awp.USC.media', dtype='f').reshape(NZ, NY, NX, 3)
#vs = para[:,:,:,1]
#rho = para[:,:,:,2]
#del para
mu_tmp = np.zeros((2 * nz, NY, NX), dtype='float32')
with open('../../data_for_Zhifeng/mesh/awp.USC.media', 'rb') as f_media:
    for i in range(2 * nz):
        data = f_media.read(4 * NY * NX * 3)
        data = np.frombuffer(data, dtype='float32').reshape(NY, NX, 3)
        mu_tmp[i, :, :] = data[:, :, 2] * data[:, :, 1] ** 2

mu = np.ones((2 * nz, 2 * nx))
for j in range(2 * nz):
        for i in range(2 * nx):
            mu[j, i] = mu_tmp[j, fault_idx[j, i, 1], fault_idx[j, i, 0]] 
