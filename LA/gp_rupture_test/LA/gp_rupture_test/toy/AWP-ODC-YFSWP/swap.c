...
void update_swapzone_buffer_x_H(float* u1, float* v1, float* w1, float* xx, float* yy, float* zz, float *xy, float *xz, float *yz, 
   float *buf_L, float *buf_R, int nyt, cudaStream_t St1, cudaStream_t St2, int rank_L, int rank_R, int zs, int ze, int meshtp);
...
void PostRecvMsg_X(float* RL_M, float* RR_M, MPI_Comm MCW, MPI_Request* request, int* count, int msg_size, int rank_L, 
   int rank_R, int gridnum)
{
	int temp_count = 0;
       
	if(rank_L>=0){
		MPI_Irecv(RL_M, msg_size, MPI_FLOAT, rank_L, MPIRANKX+rank_L*gridnum, MCW, &request[temp_count]);
		++temp_count;
	}

	if(rank_R>=0){
		MPI_Irecv(RR_M, msg_size, MPI_FLOAT, rank_R, MPIRANKX+rank_R*gridnum, MCW, &request[temp_count]);
		++temp_count;
	}
                   ...
                   int rank_L,  int rank_R,  int rank, int flag, int gridnum)
{
        ...
        if(rank_L>=0 && flag_L==1){
                MPI_Isend(SL_M, msg_size, MPI_FLOAT, rank_L, MPIRANKX+rank*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }

        if(rank_R>=0 && flag_R==1){
                MPI_Isend(SR_M, msg_size, MPI_FLOAT, rank_R, MPIRANKX+rank*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }
...
void PostRecvMsg_Y(float* RF_M, float* RB_M, MPI_Comm MCW, MPI_Request* request, int* count, int msg_size, int rank_F, int rank_B, int gridnum)
{
        ...
        if(rank_F>=0){
                MPI_Irecv(RF_M, msg_size, MPI_FLOAT, rank_F, MPIRANKY+rank_F*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }

        if(rank_B>=0){
                MPI_Irecv(RB_M, msg_size, MPI_FLOAT, rank_B, MPIRANKY+rank_B*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }
                   ...
                   int rank_F,  int rank_B,  int rank, int flag, int gridnum)
{
        ...
        if(rank_F>=0 && flag_F==1){
                MPI_Isend(SF_M, msg_size, MPI_FLOAT, rank_F, MPIRANKY+rank*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }

        if(rank_B>=0 && flag_B==1){
                MPI_Isend(SB_M, msg_size, MPI_FLOAT, rank_B, MPIRANKY+rank*gridnum, MCW, &request[temp_count]);
                ++temp_count;
        }
...
void Cpy2Host_yldfac_X(float* yldfac, float *buf_L, float *buf_R, float *d_buf_L, float *d_buf_R, int nyt, int nzt,
   ...
   msg_size = (long int) ngsl*((long int) nyt+ngsl2)*(nzt)*sizeof(float);
...
void Cpy2Host_yldfac_Y(float* yldfac, float *buf_F, float *buf_B, float *d_buf_F, float *d_buf_B, int nxt, int nzt, 
   ...
   msg_size = (long int) nxt*ngsl*nzt*sizeof(float);
...
void Cpy2Host_swaparea_X(float* u1, float* v1, float* w1, float *xx, float *yy, float *zz, float *xy, float *xz, float *yz,
    float *buf_L, float *buf_R, float *d_buf_L, float *d_buf_R, int nyt, cudaStream_t St1, cudaStream_t St2, int rank_L, int rank_R, 
   ...
   msg_size = (long int) 9*(2+ngsl+WWL)*((long int) nyt+4+ngsl2+2*WWL)*(ze-zs+1)*sizeof(float);
   /*fprintf(stdout, "msg_size=%d\n", msg_size);*/
...
void Cpy2Device_swaparea_X(float* u1, float* v1, float* w1, float *xx, float *yy, float *zz, float *xy, float *xz, float *yz,
   ...
   msg_size = 9*(2+ngsl+WWL)*((long int) nyt+4+ngsl2+2*WWL)*(ze-zs+1)*sizeof(float);
...
void Cpy2Host_swaparea_Y(float* u1, float* v1, float* w1, float *xx, float *yy, float *zz, float *xy, float *xz, float *yz,
    float *buf_F, float *buf_B, float *d_buf_F, float *d_buf_B, int nxt, cudaStream_t St1, cudaStream_t St2, int rank_F, int rank_B, 
    int zs, int ze, int meshtp){
   long int msg_size;

   update_swapzone_buffer_y_H(u1, v1, w1, xx, yy, zz, xy, xz, yz, d_buf_F, d_buf_B, nxt, St1, St2, rank_F, rank_B, zs, ze, meshtp);

   msg_size = (long int) 9*((long int) nxt+4+ngsl2)*(2+ngsl+WWL)*(ze-zs+1)*sizeof(float);
   /*fprintf(stdout, "msg_size=%d\n", msg_size);*/
...
void swaparea_update_corners(float *SL_swap, float *SR_swap, float *RF_swap, float *RB_swap, int nz, int off,
         int nxt, int nyt){
