...
void background_output_writer(int rank, int size, int nout, int wstep, int ntiskp, int ngrids, char *OUT,
   ...
   MPI_Recv(rec_nxt, ngrids, MPI_INT, rank-2*size, MPIRANKIO, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(rec_nyt, ngrids, MPI_INT, rank-2*size, MPIRANKIO+1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(rec_nzt, ngrids, MPI_INT, rank-2*size, MPIRANKIO+2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(rec_NX, ngrids, MPI_INT, rank-2*size, MPIRANKIO+3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(rec_NY, ngrids, MPI_INT, rank-2*size, MPIRANKIO+4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(rec_NZ, ngrids, MPI_INT, rank-2*size, MPIRANKIO+5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(grid_output, ngrids, MPI_INT, rank-2*size, MPIRANKIO+6, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   MPI_Recv(displacement, ngrids, MPI_OFFSET, rank-2*size, MPIRANKIO+7, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
   ...
   for (p=0; p<ngrids; p++){
	 ...
	 dispArray[p][i] = dispArray[p][i]*rec_NZ[p]*rec_NY[p]*rec_NX[p]*i;
       ...
       if(rank==master) printf("filetype size grid %d (supposedly=rec_nxt*nyt*nzt*WS*4=%ld) =%d\n", 
          p, rec_nxt[p]*rec_nyt[p]*rec_nzt[p]*wstep*sizeof(float),tmpSize);
   }
   ...
   for (n=0; n<nout; n++){
      ...
      for (p=0; p<ngrids; p++){
         if (outsize[p] > 0){
            MPI_Recv(Bufx[p], outsize[p], MPI_FLOAT, rank-2*size, MPIRANKIO+30, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(Bufy[p], outsize[p], MPI_FLOAT, rank-2*size, MPIRANKIO+31, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(Bufz[p], outsize[p], MPI_FLOAT, rank-2*size, MPIRANKIO+32, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (NVE ==3) 
               MPI_Recv(Bufeta[p], outsize[p], MPI_FLOAT, rank-2*size, MPIRANKIO+33, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
         }
      }
	 ...
	 err = MPI_File_write_all(fh, Bufx[p], rec_nxt[p]*rec_nyt[p]*rec_nzt[p]*wstep, MPI_FLOAT, &filestatus);
	 ...
	 err = MPI_File_open(MCI,filename,MPI_MODE_CREATE|MPI_MODE_WRONLY,MPI_INFO_NULL,&fh);
	 err = MPI_File_set_view(fh, displacement[p], MPI_FLOAT, filetype[p], "native", MPI_INFO_NULL);
	 err = MPI_File_write_all(fh, Bufy[p], rec_nxt[p]*rec_nyt[p]*rec_nzt[p]*wstep, MPI_FLOAT, &filestatus);
	 err = MPI_File_close(&fh);
	 sprintf(filename, "%s_%1d_%07ld", filenamebasez, p, cur_step);
	 err = MPI_File_open(MCI,filename,MPI_MODE_CREATE|MPI_MODE_WRONLY,MPI_INFO_NULL,&fh);
	 err = MPI_File_set_view(fh, displacement[p], MPI_FLOAT, filetype[p], "native", MPI_INFO_NULL);
	 err = MPI_File_write_all(fh, Bufz[p], rec_nxt[p]*rec_nyt[p]*rec_nzt[p]*wstep, MPI_FLOAT, &filestatus);
	 err = MPI_File_close(&fh);
	 ...
	 if (NVE == 3) {
	    ...
	    err = MPI_File_open(MCI,filename,MPI_MODE_CREATE|MPI_MODE_WRONLY,MPI_INFO_NULL,&fh);
	    err = MPI_File_set_view(fh, displacement[p], MPI_FLOAT, filetype[p], "native", MPI_INFO_NULL);
	    err = MPI_File_write_all(fh, Bufeta[p], rec_nxt[p]*rec_nyt[p]*rec_nzt[p]*wstep, MPI_FLOAT, &filestatus);
	    err = MPI_File_close(&fh);
	 }
      }
      MPI_Barrier(MCI);
