...
void update_yldfac_buffer_x_H(float* yldfac, float *buf_L, float *buf_R, int nyt, int nzt, cudaStream_t St1, 
...
void dstrqc2_H(float* xx, float* yy,    float* zz,    float* xy,    float* xz,  float* yz,
              ...
              cudaStream_t St, float* coeff, float *d_vx1, float *d_vx2, int *d_ww, 
              float *d_wwo, int s_i, int e_i, int s_j, int e_j, int d_i);
...
void swap_H(float * xxl, float* yyl, float* zzl, float* xyl, float* xzl, float* yzl,float* u1l, float* v1l,float* w1l,
		  ...
		  int nxtl, int nytl, float *buf_L, float *buf_R, float *buf_F, float *buf_B, int rank, 
...
void dump_variable(float *var, long int nel, char *varname, int desc, int tstep, int tsub, int rank, int ncpus){
   FILE *fid;
   char outfile[200];
...
void dump_all_vels(float *d_u1, float *d_v1, float *d_w1, long int nel, int desc, int tstep, int tsub, int rank, int ncpus){
...
void dump_local_variable(float *var, long int nel, char *varname, char desc, int tstep, int tsub, int rank, int ncpus){
   #ifdef DUMP_SNAPSHOTS
   FILE *fid;
   char outfile[200];
   ...
   fwrite(var, nel, sizeof(float), fid);
   fclose(fid);
   #endif
}

void dump_nonzeros(float *var, int nx, int ny, int nz, char *varname, int desc, int tstep, int tsub, 
     int rank, int ncpus){
   #ifdef DUMP_NONZERO
   FILE *fid;
   char outfile[200];
...
void print_nonzero_H(float *array, int nx, int ny, int nz, int d_i, int rank);
...
//  variable initialization begins 
    ...
    command(argc,argv,&TMAX,DH,&DT,&ARBC,&PHT,&NPC,&ND,NSRC,&NST,
      ...
      NBGX,NEDX,NSKPX,NBGY,NEDY,NSKPY,NBGZ,NEDZ,NSKPZ,
	    &FAC,&Q0,&EX,&FP,&IDYNA,&SoCalQ,INSRC,INVEL,OUT,INSRC_I2,CHKFILE, &ngrids, &FOLLOWBATHY);
...
rank, READ_STEP, READ_STEP_GPU, NST, IFAULT);
    ...
    if (NVE==3){
      ...
      d_neta = (float**) calloc(ngrids, sizeof(float*));
    }
