...
for j in range(nz):
    ...
    for i in range(nx):
        ...
        f_fault.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(fault_idx[2 * j, 2 * i, 0], fault_idx[2 * j, 2 * i, 1], 2 * j + 1, area / 4, mu[2 * j, 2 * i] , stk, dip, rake))
        f_fault.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(fault_idx[2 * j, 2 * i + 1, 0], fault_idx[2 * j, 2 * i + 1, 1], 2 * j + 1, area / 4, mu[2 * j, 2 * i + 1] , stk, dip, rake))
        final_sr[2 * i, 2 * j, :] = sr
    ...
    for i in range(nx):
        ...
        t1 = int(np.floor(tinit/dt))
        sr[t1 : t1+len(sliprate1)] = sliprate1.copy()
        f_fault.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(fault_idx[2 * j + 1, 2 * i, 0], fault_idx[2 * j + 1, 2 * i, 1], 2 * j + 2, area / 4, mu[2 * j + 1, 2 * i] , stk, dip, rake))
        f_fault.write("{0} {1} {2} {3} {4} {5} {6} {7}\n".format(fault_idx[2 * j + 1, 2 * i + 1, 0], fault_idx[2 * j + 1, 2 * i + 1, 1], 2 * j + 2, area / 4, mu[2 * j + 1, 2 * i + 1] , stk, dip, rake))
