...
p_rat = np.sqrt(p_ratu ** 2 + p_ratw ** 2)

rat_dep = flipud(mean(p_rat, axis=1))
...
id_w = id_w[3]
id_u = id_w
id_t = argwhere(absolute(diff(rat_dep)) < (0.005 * max(rat_dep)))
id_t = id_t[4][0]
lim_u = ratu_dep[int(round(id_t/2))]
lim_w = ratw_dep[int(round(id_t/2))]
...
ratio_w = ratio_w[::-1]
ratio_t = r0_t + np.random.normal(0, 0.02, nz) + (r_t - r0_t) * (1 -
             exp(-linspace(0, loc_t * nz / id_t, nz)))
ratio_t = ratio_t[::-1]
fs = 1 / dt
fc = 1
...
# can add a random function for each ll?
        if max(tmp) * ratio_t[kk] > lim_u and (nz-kk) <= id_t:
            ratu_t[:, kk, ll] = stf_transfer(tmp, lim_u / max(tmp), pw)
        else:
            ratu_t[:, kk, ll] = stf_transfer(tmp, ratio_t[kk], pw)

        tmp = signal.lfilter(b, a, ratw[:, kk, ll])
        if max(tmp) * ratio_t[kk] > lim_w and (nz-kk) <= id_t:
            ratw_t[:, kk, ll] = stf_transfer(tmp, lim_w / max(tmp), pw)
        else:
            ratw_t[:, kk, ll] = stf_transfer(tmp, ratio_t[kk], pw)

# output files, second replace for different rocks
output_u = open(os.path.join(os.getcwd().replace('_A', '_a')
              , "output_dyn/ratu").replace('lin','eks')
              .replace('sandstone', rock)
              ,'wb')
output_w = open(os.path.join(os.getcwd().replace('_A', '_a')
              , "output_dyn/ratw").replace('lin','eks')
              .replace('sandstone', rock)
              , 'wb')
