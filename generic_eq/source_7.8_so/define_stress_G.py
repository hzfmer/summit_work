...
mu_s=ones((nz,nx), dtype=float) * mu_stat
mu_d=ones((nz,nx), dtype=float) * mu_dyn
dmax=ones((nz,nx), dtype=float) * 10.
...
fid=open("plast_M8.0_z0_G.dat", "w")
yi=250
yz=0.
for k in range(nz-20):
   for l in range(20,nx-20):
     fid.write("%5d %5d %5d %e %e %e %e %e %e %e %e\n" % \
       (l+1, yi+1, k+1, -yy[k], xy[k], dmax[k,l], yz, mu_s[k,l], mu_d[k,l], coh[k], tnuc[k,l]))
