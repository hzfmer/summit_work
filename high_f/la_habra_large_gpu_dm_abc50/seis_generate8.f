...
C     THIS IS FOR AVERAGING ACROSS THE FREE-SURFACE
      ...
      real, dimension (:), allocatable :: xdata, xdata_t
      real, dimension (:), allocatable :: ydata, ydata_t
      real, dimension (:), allocatable :: zdata, zdata_t
...
C.0   Get argument first
     ...
     +            int(totalsteps_f/ntiskp)
      ...
      allocate(xdata_t(nx*ny))
      allocate(ydata_t(nx*ny))
      allocate(xs(numsta))
      allocate(ys(numsta))
...
c         read z=0 plane and z=1 plane as seperate slices
          read(9,rec=nz*p-1) xdata_t
          read(9,rec=p*2) xdata
          read(10,rec=nz*p-1) ydata_t
          read(10,rec=p*2) ydata
c          read(11,rec=nz*p-1) zdata_t
          read(11,rec=p*2) zdata
          do m=1,numsta
...
222   continue
      ...
      print *, "seis_x"
      write(fseis, '(A6,I7.7)')
     +       'seis8_x',current_save_block*save_step*fileunit
     ...
     + access='direct',recl=floatsize*numsta*write_step*save_step)
      ...
      print *, "seis_y"
      write(fseis, '(A6,I7.7)')
     +        'seis8_y',current_save_block*save_step*fileunit
      open(19,file=fseis,form='unformatted',
     +  access='direct',recl=floatsize*numsta*write_step*save_step)
      ...
      print *, "seis_z"
      write(fseis, '(A6,I7.7)')
     +         'seis8_z',current_save_block*save_step*fileunit
      open(19,file=fseis,form='unformatted',
     + access='direct',recl=floatsize*numsta*write_step*save_step)
