...
program add
  ...
  ! small
  integer(kind=8), parameter    :: nx = 3456, ny = 3456, nz = 108
  ...
  character(len=70)             :: filename_fr, filename_hom, filename_het
  ! small
  real, parameter               :: vpmax = 3850., vsmax = 2722., vsmin=100.    
  
  filename_fr = 'ssh_8m_r05h005l100.out'
  filename_hom = '../la_habra_small_8m_gpu_dm_abc50/mesh_orig_p0.bin'
  filename_het = '../la_habra_small_8m_gpu_dm_abc50/mesh_s10h005l100_p0.bin'
  ...
  do j=1,ny
    do i=1,nx
      ...
      ind = (j-1)*nx+i
      if (rank >= nz) then
          buf_het(3*(ind-1)+1)=buf_hom(3*(ind-1)+1)
          buf_het(3*(ind-1)+2)=buf_hom(3*(ind-1)+2)
          buf_het(3*(ind-1)+3)=buf_hom(3*(ind-1)+3)
      else
          ...
          if (temp_vs > 0.) then
             ...
             if (buf_het(3*(ind-1)+2) .lt. vsmin) then
                buf_het(3*(ind-1)+2) = vsmin
                if (buf_het(3*(ind-1)+1) .lt. vsmin*sqrt(2.)) then
                   buf_het(3*(ind-1)+1) = vsmin*sqrt(2.)
                endif
             endif
             buf_het(3*(ind-1)+3) = temp_rho*(1+std*buf_fr(ind))
          else
             buf_het(3*(ind-1)+1)=temp_vp
             buf_het(3*(ind-1)+2)=temp_vs
