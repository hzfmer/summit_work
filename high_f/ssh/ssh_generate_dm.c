...
int main(int argc, char **argv) {
	...
	printf("Finished pow3iso, time=%4.3f s\n", gethrtime() - t_init);
    fflush(stdout);
	...
	for (iz = 0; iz < n1; iz++) {
		...
		printf("Calculated interpft for index %d, time=%4.3f s\n", iz, gethrtime() - t_init);
        fflush(stdout);
		...
		for (j = 0; j < n3; j++) {
			for (k = 0; k < n2; k++) {
				holder = powxy[j][k][0];
                // Previously was 1, sizeof(float), wrong but doesn't affect.
