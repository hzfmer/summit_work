...
int pow3iso(double dx, double hurst, double l1, double seed, double n1iso_bigger, double n2iso_bigger,
    ...
    fprintf(stdout, "Size of sim3d is %zu\n", totalsize * sizeof(fftw_complex));
    fflush(stdout);
	...
	for (j = 0; j < n3iso_bigger; j++) {
		...
		for (i = 0; i < n1iso_bigger; i++) {
			for (k = 0; k < n2iso_bigger; k++) {
				...
				//ARRs3d(sim3d, i, k, j)[0] = ampspec[i][k] * randn(); 
				//ARRs3d(sim3d, i, k, j)[1] = ampspec[i][k] * randn();
			}
		}
	}
...
int main(int argc, char **argv) {
	...
	for (iz = 0; iz < n1; iz++) {
		...
		for (j = 0; j < n3; j++) {
			for (k = 0; k < n2; k++) {
				holder = powxy[j][k][0];
				fwrite(&holder, sizeof(float), 1, fp);	
			}
		}
