...
def generate_files(conf_file='param.sh'):
    ...
    args = parser.parse_known_args([f'@{conf_file}'])[0]
    ...
    with open('IN3D_gmrot.out', 'w') as f_in3dgmrot, open('IN3D.out', 'w') as f_in3dout:
        for key, value in vars(args).items():
            ...
            if key == 'WRITE_STEP' and vars(args)['IVELOCITY'] == 0:
                # IVELOCITY = 1 means aggregative output, which infers WRITE_STEP required by gmrot
